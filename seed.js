...
const serviceAccount = JSON.parse(readFileSync(new URL('./firebase-key.json', import.meta.url), 'utf-8'));
...
async function main() {
  ...
  const textMessages = [
    ...
    },
    {
      ...
      type: "about",
      message:
        "In addition to individual instruction, group classes are held periodically, often before recitals, to build ensemble skills and confidence. Students also have the option to participate in events such as the Federation piano competition for performance and evaluation opportunities.",
    },
    {
    ...
    },
    {
      ...
      type: "header",
      message:
        "Check here for scheduling purposes, recital info, and links to resources you will use throughout the year.",
    },
    {
      name: "contact_about",
      type: "contact",
  ...
  console.log('Seeding text messages...');
  const batch = db.batch();
  textMessages.forEach((msg) => {
    const docRef = db.collection('text').doc(); // auto-ID
    batch.set(docRef, msg);
  });
  await batch.commit();
  console.log('✅ Text messages inserted.');
  ...
  console.log('🎉 Seed data inserted into Firestore successfully.');
}
